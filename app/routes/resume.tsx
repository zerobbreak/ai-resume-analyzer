...
export const meta = () => [
  ...
  { description: "Review your resume with AI" },
];
...
const Resume = () => {
  ...
  useEffect(() => {
    const loadResume = async () => {
      try {
        ...
        // Redirect HR reviews to their dedicated page
        if (data.analysisType === "hr-review") {
          navigate(`/hr-review/${id}`);
          return;
        }
  ...
  const handleImprove = async () => {
    ...
    try {
      // Collect all "improve" tips from feedback
      const improveTips: {
        category: string;
        tip: string;
        explanation: string;
      ...
      const categories = [
        { key: "toneAndStyle", name: "Tone & Style" },
      ...
      categories.forEach(({ key, name }) => {
        const categoryFeedback = feedback[key as keyof Feedback];
        if (
          categoryFeedback &&
